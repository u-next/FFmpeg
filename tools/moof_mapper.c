 ...
 *
 * moof_mapper -path-prefix bar a.ismv b.ismv c.isma
 * This reads a.ismv, b.ismv and c.isma from directory bar/.
 * The moof offset map is written to stdout.
 *
...
{
    fprintf(stderr, "%s [-path-prefix prefix] file1 [file2] ...\n", argv0);
    return ret;
}
...
static int write_fragment_map(struct Tracks *tracks, int start_index, AVIOContext *in)
{
    ...
    for (i = start_index; i < tracks->nb_tracks; i++) {
        ...
        for (j = 0; j < track->chunks; j++) {
            ...
            fprintf(stdout, "%s: %s/Fragments(%s=%"PRId64") %"PRId64,
                    track->name, qualitylevel_str, type,
                    track->offsets[j].time, avio_tell(in));
...
static int handle_file(struct Tracks *tracks, const char *path_prefix, const char *file)
{
    ...
    err = write_fragment_map(tracks, orig_tracks, f);
    if (err < 0) {
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        if (!strcmp(argv[i], "-path-prefix")) {
            ...
            if (handle_file(&tracks, path_prefix, argv[i])) {
                return 1;
            }
        }
    }
